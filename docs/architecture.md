...
The bridge keeps the model worker warm. An `EngineManager` loads the completion model — and, when delegating, the peer link — once and reuses it for every turn with the same configuration, keeping the knowledge base open too; independent turns stay isolated with `kvCache: false` at completion time, so there is no context bleed and no overflow without a worker teardown. The first turn pays the cold load and, for a peer, the DHT holepunch; subsequent turns skip both. This is what makes the live voice loop responsive.

Live voice is a streaming pipeline on the same warm worker: the browser captures the mic and streams 16 kHz PCM to the bridge, where a Whisper session with the Silero VAD model does its own voice-activity detection and silence endpointing (`core/voicestream.ts`); on end-of-turn the warm grounded model answers and Supertonic streams the answer back as PCM, which the browser plays gaplessly. Barge-in keeps scoring the mic while speaking and cancels the in-flight speech when the user talks over it. The mesh panel adds provider mode — the bridge can run `startQVACProvider` in-process (rebinding the worker to a deterministic seed) so this device serves a model to peers — and records real per-peer served-turn stats and the last routing decision for the readout.
